...
folder__ = "/home/gpfs/manip/mnt0607/bao/hdumasde/Code/CrossCorrelation/Python/All_Spectrum/Iterative_spectrum/Run2/"
...
def make_all_Fits(iStart=0,iEnd=-1,step=0):
	...
	binEdge_lobs = numpy.arange(0.,15001.,1.)
	xxx_lobs = numpy.arange(0.,15000.,1.)
	yyy_lobs = numpy.zeros( xxx.size )
	eyy_lobs = numpy.zeros( xxx.size )
	www_lobs = numpy.zeros( xxx.size )
	nbb_lobs = numpy.zeros( xxx.size )


	### Template
	template = numpy.loadtxt(folder__+'/template_step_'+str(step)+'.txt')
	template = interpolate.interp1d(template[:,0],template[:,1],bounds_error=False,fill_value=1)
	template_lobs = numpy.loadtxt(folder__+'/template_lobs_step_'+str(step)+'.txt')
	template_lobs = interpolate.interp1d(template_lobs[:,0],template_lobs[:,1],bounds_error=False,fill_value=1)
	...
	for el in cat_tbhdu:
		...
		a0,a1 = fit_spectrum(flux,model,weigh,lamRF)
		if ((a0+a1*lamRF)[ (a0+a1*lamRF)<=0. ].size>0.):
		...
		print a0, a1
		plt.plot(lamRF,flux)
		plt.plot(lamRF,(a0+a1*lamRF)*model,linewidth=4)
		#plt.plot(lamRF,(a0+a1*lamRF)*model*template_lobs(lamObs))
		plt.grid(True, which='both')
		plt.show()
		'''

		#delta = flux/( (a0+a1*lamRF)*model*template_lobs(lamObs))-1.
		delta = flux/( (a0+a1*lamRF)*model)-1.
		number, axisX, axisY = numpy.histogram2d(lamRF, delta, (binEdge,1), weights=numpy.ones(delta.size))
		weight, axisX, axisY = numpy.histogram2d(lamRF, delta, (binEdge,1), weights=weigh)
		mean,   axisX, axisY = numpy.histogram2d(lamRF, delta, (binEdge,1), weights=weigh*delta)
		err,    axisX, axisY = numpy.histogram2d(lamRF, delta, (binEdge,1), weights=weigh*(delta**2.))
		yyy += mean[:,0]
		eyy += err[:,0]
		www += weight[:,0]
		nbb += number[:,0]
		
		number, axisX, axisY = numpy.histogram2d(lamObs, delta, (binEdge,1), weights=numpy.ones(delta.size))
		weight, axisX, axisY = numpy.histogram2d(lamObs, delta, (binEdge,1), weights=weigh)
		mean,   axisX, axisY = numpy.histogram2d(lamObs, delta, (binEdge,1), weights=weigh*delta)
		err,    axisX, axisY = numpy.histogram2d(lamObs, delta, (binEdge,1), weights=weigh*(delta**2.))
		yyy_lobs += mean[:,0]
		eyy_lobs += err[:,0]
		www_lobs += weight[:,0]
		nbb_lobs += number[:,0]
	...
	numpy.savetxt(folder__+'/spectrum_new_technic_lobs_step_'+str(step)+'_'+str(iStart)+'_'+str(iEnd)+'.txt',zip(xxx_lobs, yyy_lobs, eyy_lobs, www_lobs, nbb_lobs))
...
def get_delta(step):
	...
	raw_scheme = 'spectrum_new_technic_lobs_step_'+str(step)+'_'
	...
	cut = nbb>0.
	ddd[cut] = yyy[cut]/www[cut]
	numpy.savetxt(folder__+'/residual_lobs_step_'+str(step)+'.txt',zip(xxx, ddd))
...
def get_new_spectrum(step):
	...
	if (step==0):
		...
		ddd           = numpy.zeros(xxx.size)
		###
		template_lobs      = numpy.ones( (xxx_lobs.size,2) )
		template_lobs[:,0] = xxx_lobs
		ddd_lobs           = numpy.zeros(xxx_lobs.size)
	else:
		...
		ddd_lobs            = numpy.loadtxt(folder__+'/residual_lobs_step_'+str(step-1)+'.txt')[:,1]
		template_lobs       = numpy.loadtxt(folder__+'/template_lobs_step_'+str(step-1)+'.txt')
		template_lobs[:,1] *= ddd_lobs+1.
		### get index of first and last not empty pixels
		idx   = numpy.arange(0.,15000.,1.).astype('int')
		idx2  = idx[ (ddd_lobs!=0.) ]
		first = idx2[0]
		last  = idx2[-1]
		template_lobs[:,1][ (idx<first) ] = template_lobs[first,1]
		template_lobs[:,1][ (idx>last) ]  = template_lobs[last,1]
		### Set values inside the spectrum without pixels
		tmp_template = interpolate.interp1d(template_lobs[:,0][(template_lobs[:,1]!=1.)],template_lobs[:,1][(template_lobs[:,1]!=1.)],bounds_error=False,fill_value=1)
		template_lobs[:,1][(template_lobs[:,1]==1.)] = tmp_template(template_lobs[:,0][(template_lobs[:,1]==1.)])
		### No pixels less or equal to zero
		template_lobs[:,1][ (template_lobs[:,1]<=0.01) ] = 0.01

	plt.plot(xxx, template[:,1] )
	plt.show()
	plt.plot(xxx, ddd )
	plt.show()
	plt.plot(xxx_lobs, template_lobs[:,1] )
	plt.show()
	plt.plot(xxx_lobs, ddd_lobs )
	plt.show()

	numpy.savetxt(folder__+'/template_step_'+str(step)+'.txt',zip(template[:,0], template[:,1]))
	numpy.savetxt(folder__+'/template_lobs_step_'+str(step)+'.txt',zip(template_lobs[:,0], template_lobs[:,1]))
...
def plot_all_spectrum():
	...
	### template
	for i in range(1,step_nb):
		template = numpy.loadtxt(folder__+'/template_lobs_step_'+str(i)+'.txt')
		plt.plot(xxx, template[:,1] )
	plt.show()

	### residuals
	for i in range(step_nb-1):
		template = numpy.loadtxt(folder__+'/residual_lobs_step_'+str(i)+'.txt')
		plt.plot(xxx, template[:,1] )
	plt.show()
